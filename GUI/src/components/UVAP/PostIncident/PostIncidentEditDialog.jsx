...
import { FILE_MAX_LENGTH, VapNoChangeInfoMsg } from 'components/UVAP/util';
...
function PostIncidentEditDialog(props) {
  const classes = useStyles();
  const { loading, onClose, onSave, open, incidentDetails } = props;
  const dialogTitle = I18n.t('vap.dialog.incident.editTitle');
  ...
  useEffect(() => {
    if (!_.isEmpty(incidentDetails)) {
      setOldData(incidentDetails);
      dispatchNewData({ type: '', data: incidentDetails });
    }
  }, [incidentDetails]);
  // init func
...
PostIncidentEditDialog.defaultProps = {
  open: false,
  onClose: () => {},
  onSave: () => {},
  incidentDetails: {}
...
PostIncidentEditDialog.propTypes = {
  open: PropTypes.bool,
...
export default PostIncidentEditDialog;
