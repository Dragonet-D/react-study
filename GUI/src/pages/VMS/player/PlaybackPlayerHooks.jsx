...
class Player extends React.Component {
  ...
  $bookMarkToggle = bool => {
    this.setState({ bookMarkToggle: bool });
  };
  ...
  initPlayer(_playTime, type) {
    ...
    this.handleMessage({ type: 'disableSkip' });
    ...
    this.player.on('ready', e => {
      this.handleMessage({ type: 'enableSkip' });
    });
    if (type === 'start') {
  ...
  skip(_skip, ts) {
    // const skipTime = Math.floor((this.ts - this.startTime) / 1000 + _skip);
    const skipTime = ts + _skip;

    if (skipTime < 0) {
      // this.pause();
      this.ts = 1;
      msg.info('Video play from beginning');
      this.initPlayer(0);
    } else {
      // this.pause();
      this.ts = 1;
      this.initPlayer(skipTime);
    }
  }

  bookmarkPause = () => {
    this.handleMessage({ type: 'bookmarkPause' });
  };
            ...
            >
              ...
              {mute ? (
                <VolumeOff
                  ...
                  style={{ marginTop: '18px', position: 'absolute', top: '76px' }}
                />
              ) : (
                <VolumeUp
                  className={classes.clickBut}
