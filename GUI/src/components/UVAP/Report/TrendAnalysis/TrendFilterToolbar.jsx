...
import { DatePicker, BasicLayoutTitle, TextField, SingleSelect, Button } from 'components/common';
...
function TrendFilterToolbar(props) {
    ...
    switch (action.type) {
      ...
      case 'timeunit':
        if (_.isNil(action.data) || action.data === '') {
          ...
          return { ...params, timeunit: action.data };
        }
      case 'timezone':
        if (_.isNil(action.data) || action.data === '') {
          ...
          return { ...params, timezone: action.data };
        }
      case 'reset':
        return {
  ...
  useEffect(() => {
    ...
    // const timeunitStatus = _.isNil(params.timeunit) || params.timeunit === '';
    // const timezoneStatus = _.isNil(params.timezone) || params.timezone === '';
    // // setFilterBtnStatus(
    //   typeStatus || instanceTypeStatus || sortStatus || timeunitStatus || timezoneStatus
    // );
  ...
  return (
    <Fragment>
      <BasicLayoutTitle titleName={I18n.t('menu.uvap.children.report.children.trendAnalysis')}>
        <IconButton
          size="small"
      ...
      <VapCollapseBox filterStatus={searchContent}>
        ...
        {/* row 3 */}
        <Grid container spacing={3} alignItems="center" className={classes.itemsRow}>
          <Grid item className={classes.itemsStyle}>
            <DatePicker
              value={params.timefr}
              handleChange={val => {
          ...
          {/* <Grid item className={classes.itemsStyle}>
            <SingleSelect
              label={I18n.t('vap.toolbar.report.timeunit')}
              selectOptions={VAP_COMMON.timeunit}
              onSelect={val => dispatchParams({ type: 'timeunit', data: val })}
              value={params.timeunit || ''}
              error={timeunitError}
              errorMessage={timeunitError ? I18n.t('vap.toolbar.report.timeunitErrorMsg') : ''}
              fullWidth
              required
            />
          </Grid> */}
          {/* <Grid item className={classes.itemsStyle}>
            <SingleSelect
              label={I18n.t('vap.toolbar.report.timezone')}
              selectOptions={VAP_COMMON.timezone}
              onSelect={val => dispatchParams({ type: 'timezone', data: val })}
              value={params.timezone || ''}
              error={timezoneError}
              errorMessage={timezoneError ? I18n.t('vap.toolbar.report.timezoneErrorMsg') : ''}
              fullWidth
              required
            />
          </Grid> */}
          ...
          <Grid item className={classes.itemsStyle}>
            <Button onClick={() => getReportList()}>{I18n.t('global.button.filter')}</Button>
            <Button
              onClick={() => {
                clearTableList();
...
export default TrendFilterToolbar;
