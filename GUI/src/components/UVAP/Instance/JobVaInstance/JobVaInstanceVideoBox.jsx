...
import PlayerUltra from 'pages/VMS/player/PlayerComponent';
// list
...
class JobVaInstanceVideoBox extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!_.isEqual(this.props, nextProps)) {
      ...
      if (rProps.playbackData) {
        if (!_.isEqual(instance.state.streamInfo, rProps.playbackData)) {
  ...
  handleMessageFromPlayer = msg => {
    ...
    switch (msg.type) {
      ...
      case 'receivedConfig':
        playerRef[`player${msg.id}`].videoStart = _.toNumber(from);
        this.dispatch({
          type: 'VMSPlayback/requestPlayback',
          payload: {
            ...msg.data,
            userId: global.userId,
            from: moment(from).format(DATE_FORMAT_DATE_T_TIME),
            to: moment(to).format(DATE_FORMAT_DATE_T_TIME)
          },
          id: msg.id
        });
        break;
...
export default withStyles(styles)(
  connect(({ global, JobVaInstance, VMSPlayback, loading }) => ({
    global,
    JobVaInstance,
    VMSPlayback,
    loading
